...
tillphi = np.zeros_like(X) + 30
tillphi[hudson_bay] = 5
tillphi[hudson_strait] = 5
...
for exp in experiments:
    ...
    write_netcdf(filename, x, y, topg, thk, land, smb, surface_temp, tillphi)

# b_min = 0.15  # m ice equivalent per year
# b_max = 0.30  # m ice equivalent per year
# smb = b_min + (b_max - b_min) / radius * distance_from_center
# smb[ocean] = 0

# T_min = 233.15  # Kelvin
# S_T = 2.5e-9  # Kelvin per kilometer cubed
# surface_temp = T_min + S_T * distance_from_center**3


# plt.imshow(land)
# plt.imshow(hudson_bay)
# plt.imshow(hudson_strait)
# plt.imshow(smb)
# plt.imshow(surface_temp - 273.15)
# plt.imshow(tillphi)
# plt.show()
#
